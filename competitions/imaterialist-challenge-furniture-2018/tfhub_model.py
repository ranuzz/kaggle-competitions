...
def tfhub_model_fn(features, labels, mode):
    module = hub.Module(TF_HUB_MODULE_URL)
    images = tf.reshape(features["image_data"], [-1, IMAGE_X, IMAGE_Y, IMAGE_LAYER])
    module_out = module(images)  # Features with shape [batch_size, num_features].
    # Logits Layer
    logits = tf.layers.dense(inputs=module_out, units=256)
...
def main(unused_argv):
    ...
    furniture_classifier = tf.estimator.Estimator(
        model_fn=tfhub_model_fn, model_dir="./tensorflow_models")
...
if __name__ == "__main__":
    ...
    TF_MODULE = hub.Module(TF_HUB_MODULE_URL)
    IMAGE_X, IMAGE_Y = hub.get_expected_image_size(TF_MODULE)

    print("Loaded TF_MODULE : ", TF_HUB_MODULE_URL)
    print("Expected (H,W) ", IMAGE_X, IMAGE_Y)
